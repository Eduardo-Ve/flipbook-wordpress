!(function (t, e) {
  ...
  class i extends e {
    ...
    draw(t) {
      ...
      const s = this.render.getRect().pageWidth;
      const n = this.render.getRect().height;
    ...
    simpleDraw(t) {
      ...
      const i = this.render.getContext();
      const s = e.pageWidth;
      const n = e.height;
      const h = 1 === t ? e.left + e.pageWidth : e.left;
      const r = e.top;
  ...
  class h {
    ...
    static GetIntersectBeetwenTwoLine(t, e) {
      ...
      const n = t[1].x - t[0].x;
      const h = e[1].x - e[0].x;
      const r = t[0].x * t[1].y - t[1].x * t[0].y;
      const o = e[0].x * e[1].y - e[1].x * e[0].y;
      const a = i * o - s * r;
      const g = n * o - h * r;
      const l = -(r * h - o * n) / (i * h - s * n);
      const d = -(i * o - s * r) / (i * h - s * n);
  ...
  class r extends e {
    ...
    newTemporaryCopy() {
      if ("hard" === this.nowDrawingDensity) return this;

      if (null === this.temporaryCopy) {
    ...
    drawHard(t = "") {
      ...
      const i = this.state.hardDrawingAngle;
      const s =
        t +
    ...
    simpleDraw(t) {
      ...
      const i = e.pageWidth;
      const s = e.height;
      const n = 1 === t ? e.left + e.pageWidth : e.left;
      const h = e.top;
  ...
  class a {
    ...
    calcAngleAndPosition(t) {
      ...
      if (Math.abs(e.x - this.pageWidth) < 1 && Math.abs(e.y) < 1)
    ...
    calculateAngle(t) {
      ...
      const i = "bottom" === this.corner ? this.pageHeight - t.y : t.y;
    ...
    checkPositionAtCenterLine(t, e, i) {
      ...
      let a = this.rect.topLeft;

      if ("bottom" === this.corner) {
        o = this.rect.topRight;
  ...
  class g {
    ...
    flip(t) {
      ...
      const i = e.height / 10;
      const s = "bottom" === this.calc.getCorner() ? e.height - i : i;
      const n = "bottom" === this.calc.getCorner() ? e.height : 0;

      this.calc.calc({ x: e.pageWidth - i, y: s });
      this.animateFlippingTo({ x: e.pageWidth - i, y: s }, { x: -e.pageWidth, y: n }, !0);
    }
    ...
    start(t) {
      ...
      const i = this.getBoundsRect();
      const s = this.getDirectionByPoint(e);
        ...
        if ("landscape" === this.render.getOrientation()) {
          if (1 === s) {
            ...
            null !== t &&
              ...
              (this.flippingPage.setDrawingDensity("hard"), t.setDrawingDensity("hard"));
          } else {
    ...
    flipToPage(t, e) {
      ...
      const s = this.app.getPageCollection().getSpreadIndexByPage(t);
    ...
    stopMove() {
      ...
      const e = this.getBoundsRect();
      const i = "bottom" === this.calc.getCorner() ? e.height : 0;
    ...
    isPointOnCorners(t) {
      const e = this.getBoundsRect();
      const i = e.pageWidth;
      const s = Math.sqrt(Math.pow(i, 2) + Math.pow(e.height, 2)) / 5;
  ...
  class l {
    ...
    calculateBoundsRect() {
      ...
      const n = this.setting.width / this.setting.height;

      let h = this.setting.width;
      let r = this.setting.height;
      let o = i - h;

      if ("stretch" === this.setting.size) {
        ...
        if (h > this.setting.maxWidth) {
          h = this.setting.maxWidth;
          r = h / n;
          if (r > this.getBlockHeight()) {
        ...
        if (e < 2 * h && this.app.getSettings().usePortrait) {
          t = "portrait";
          o = i - h / 2 - h;
        }
      }
    ...
    setShadowData(t, e, i, s) {
      ...
      this.shadow = {
        pos: t,
        angle: e,
        width: (3 * this.getRect().pageWidth * i) / 4 / 100,
  ...
  class d extends l {
    ...
    drawInnerShadow() {
      ...
      const e = this.convertToGlobal({ x: this.shadow.pos.x, y: this.shadow.pos.y });
      const i = this.convertRectToGlobal(this.pageRect);
      ...
      const n = this.ctx.createLinearGradient(0, 0, s, 0);
      ...
      this.onTouchEnd = (t) => {
        if (t.changedTouches.length > 0) {
          ...
          if (null !== this.touchPoint) {
            ...
            const e = Math.abs(i.y - this.touchPoint.point.y);
  ...
  class u extends p {
    ...
    resizeCanvas() {
      ...
      const e = parseInt(t.getPropertyValue("width"), 10);
      const i = parseInt(t.getPropertyValue("height"), 10);
  ...
  class w extends l {
    ...
    drawHardInnerShadow() {
      const t = this.getRect();
      const e = this.shadow.progress > 100 ? 200 - this.shadow.progress : this.shadow.progress;
    ...
    drawHardOuterShadow() {
      ...
      let e =
        (100 - (this.shadow.progress > 100 ? 200 - this.shadow.progress : this.shadow.progress)) *
          ((2.5 * t.pageWidth) / 100) +
        20;
    ...
    drawInnerShadow() {
      const t = this.getRect();
      const e = (3 * this.shadow.width) / 4;
      const i = 0 === this.getDirection() ? e : 0;
      const s = 0 === this.getDirection() ? "to left" : "to right";
      const n = this.convertToGlobal(this.shadow.pos);
      const r = this.shadow.angle + (3 * Math.PI) / 2;
      const o = [
    ...
    drawOuterShadow() {
      const t = this.getRect();
      const e = this.convertToGlobal({ x: this.shadow.pos.x, y: this.shadow.pos.y });
      const i = this.shadow.angle + (3 * Math.PI) / 2;
      const s = 1 === this.getDirection() ? this.shadow.width : 0;
      const n = 0 === this.getDirection() ? "to right" : "to left";
      const r = [
        { x: 0, y: 0 },
        { x: t.pageWidth, y: 0 },
    ...
    drawBottomPage() {
      ...
      if ("portrait" === this.orientation && 1 === this.direction) return;
    ...
    drawFrame() {
      ...
      if (null != this.flippingPage) {
        this.flippingPage.getElement().style.zIndex = (this.getSettings().startZIndex + 5).toString(
          10
        );
        this.flippingPage.draw();
      }

      if (null != this.shadow && null !== this.flippingPage) {
        if ("soft" === this.flippingPage.getDrawingDensity()) {
    ...
    clear() {
      for (const t of this.app.getPageCollection().getPages()) {
        if (t !== this.leftPage && t !== this.rightPage && t !== this.flippingPage && t !== this.bottomPage) {
  ...
  t.PageFlip = class extends (class {
    ...
    userMove(t, e) {
      if (this.isUserTouch || e || !this.setting.showPageCorners) {
        if (this.isUserTouch && h.GetDistanceBetweenTwoPoint(this.mousePosition, t) > 5) {
          this.isUserMove = !0;
          this.flipController.fold(t);
        }
      } else {
    ...
    userStop(t, e = !1) {
      if (this.isUserTouch) {
        ...
        this.isUserMove ? this.flipController.stopMove() : this.flipController.flip(t);
      }
    }
  };
