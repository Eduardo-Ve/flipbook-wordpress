    ...
    if (!empty($public_key) && !empty($secret_key)) {
        ...
        if ($test_result['success']) {
            $api_status = '<div style="background:#d4edda;border-left:4px solid #28a745;padding:12px;margin:15px 0;border-radius:4px;">
                <strong style="color:#155724;">Conexión API exitosa</strong>
            </div>';
        } else {
        ...
        <form method="post" action="options.php">
            ...
            <table class="form-table">
                <tr>
                    ...
                    <td>
                        <input type="text" name="fbw_ilovepdf_public_key" class="regular-text"
                            value="<?php echo esc_attr($public_key); ?>" placeholder="project_public_xxxxx" />
                        <p class="description">Obtén la API Key <a href="https://developer.ilovepdf.com/" target="_blank">Aquí</a></p>
                    </td>
                </tr>
                <tr>
                    <th scope="row">Secret Key</th>
                    <td>
                        <input type="password" id="fbw_secret_key" name="fbw_ilovepdf_secret_key" class="regular-text"
                            value="<?php echo esc_attr($secret_key); ?>" placeholder="secret_key_xxxxx" />
                        <button type="button" class="button" onclick="
                            var input = document.getElementById('fbw_secret_key');
                            input.type = input.type === 'password' ? 'text' : 'password';
                            this.textContent = input.type === 'password' ? 'Mostrar' : 'Ocultar';
                        ">Mostrar</button>
                        <p class="description">Tu Secret Key generada</p>
                    </td>
                </tr>
            </table>
            ...
            <table class="form-table">
                <tr>
                    ...
                    <td>
                        ...
                        <p class="description">Aplica a cualquier PDF subido a WordPress, no solo flipbooks</p>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <select name="fbw_compression_level">
                            <option value="low" <?php selected($compression_level, 'low'); ?>>Baja - Máxima calidad, menos compresión</option>
                            <option value="recommended" <?php selected($compression_level, 'recommended'); ?>>Recomendada - Balance óptimo (predeterminado)</option>
                            <option value="extreme" <?php selected($compression_level, 'extreme'); ?>>Extrema - Máxima compresión, menor calidad</option>
                        </select>
                        <p class="description"><strong>Recomendada</strong> ofrece el mejor balance entre tamaño y calidad</p>
                    </td>
                </tr>
                <tr>
                    <th scope="row">Tamaño Máximo</th>
                    <td>
                        <input type="number" name="fbw_max_file_size" value="<?php echo esc_attr($max_file_size); ?>"
                            min="1" max="200" style="width:100px;" /> MB
                        <p class="description">PDFs mayores a este tamaño no se comprimirán automáticamente</p>
                    </td>
                </tr>
                <tr>
                    <th scope="row">Tamaño Mínimo</th>
                    <td>
                        <input type="number" name="fbw_min_file_size" value="<?php echo esc_attr($min_file_size); ?>"
                            min="1" max="200" style="width:100px;" /> MB
                        <p class="description">PDFs menores a este tamaño no se comprimirán</p>
                    </td>
                </tr>
            </table>
...
function fbw_test_api_connection($public_key, $secret_key) {
    try {
        $response = wp_remote_post('https://api.ilovepdf.com/v1/auth', [
            'body'    => json_encode(['public_key' => $public_key]),
...
function fbw_compress_pdf_with_ilovepdf($upload) {
    ...
    if ($original_size < $min_size) {
        error_log(sprintf('FBW: PDF demasiado pequeño (%s) - Mínimo: %s', size_format($original_size), size_format($min_size)));
        return $upload;
    }

    error_log(sprintf('FBW: Iniciando compresión de PDF (%s) - %s', basename($original_file), size_format($original_size)));
    ...
    try {
        ...
        if ($compressed_file && file_exists($compressed_file)) {
            ...
            error_log(sprintf('FBW: Resultado - Original: %s | Comprimido: %s | Ahorro: %d%%',
                size_format($original_size), size_format($compressed_size), $savings_percent));

            if ($savings_percent > 5) {
                if (rename($compressed_file, $original_file)) {
                    error_log(sprintf('FBW: PDF comprimido exitosamente - Ahorro: %d%% (%s -> %s)',
                        $savings_percent, size_format($original_size), size_format($compressed_size)));
...
function fbw_ilovepdf_compress($file_path, $public_key, $secret_key) {
    ...
    if ($start_code === 404) {
        ...
        $start_response = wp_remote_post('https://api.ilovepdf.com/v1/task', [
            ...
            'body'    => json_encode(['tool' => 'compress']),
            'timeout' => 30
        ]);
    ...
    $result = function_exists('curl_init')
        ? fbw_upload_with_curl($upload_url, $token, $task_id, $file_path)
        : fbw_upload_with_wp_remote($upload_url, $token, $task_id, $file_path);
    ...
    if ($process_code !== 200) {
        throw new Exception('Error HTTP ' . $process_code . ' al procesar: ' . wp_remote_retrieve_body($process_response));
    }
    ...
    if ($download_code !== 200) {
        throw new Exception('Error HTTP ' . $download_code . ' al descargar: ' . wp_remote_retrieve_body($download_response));
    }
...
function fbw_upload_with_curl($upload_url, $token, $task_id, $file_path) {
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_POST           => true,
        CURLOPT_POSTFIELDS     => ['task' => $task_id, 'file' => $cfile],
    ...
    if (!$compressed) {
        ...
        $fields['fbw_compress'] = [
            ...
            'html'  => '<div style="background:#d4edda;padding:10px;border-radius:4px;border-left:4px solid #28a745;">
                ...
                <small style="color:#155724;">Ahorro: ' . $savings . '% (' . size_format($original_size) . ' &rarr; ' . size_format($compressed_size) . ')</small>
            </div>'
        ];
    }
...
function fbw_render_metabox($post) {
    ...
    $pdf_name = $pdf_url ? basename($pdf_url) : '';
    ?>
        ...
        <div class="fbw-pdf-selector">
            ...
            <div class="fbw-pdf-preview" style="margin-top:10px;<?php echo !$pdf_id ? 'display:none;' : ''; ?>">
                <div style="background:#f0f0f0;padding:10px;border-radius:4px;border:1px solid #ddd;">
                    <p style="margin:0;font-size:12px;color:#666;"><strong>Archivo seleccionado:</strong></p>
                    <p style="margin:5px 0 0;font-size:11px;word-break:break-all;">
                        <span class="fbw-pdf-name"><?php echo esc_html($pdf_name); ?></span>
                    </p>
                    <a href="#" class="fbw-remove-pdf button button-small" style="margin-top:8px;color:#d63638;">Quitar PDF</a>
                </div>
                <div style="margin-top:10px;padding:10px;background:#e7f3ff;border-radius:4px;">
                    <iframe class="fbw-pdf-iframe"
                        src="<?php echo esc_url($pdf_url); ?>#toolbar=0&navpanes=0&scrollbar=0"
                        style="width:100%;height:200px;border:1px solid #ddd;border-radius:3px;"></iframe>
                </div>
            </div>
        </div>
        <div style="margin-top:20px;">
            <label style="display:block;margin-bottom:5px;font-weight:bold;">Dimensiones del Flipbook:</label>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                <div>
                    <label style="font-size:11px;display:block;margin-bottom:3px;">Ancho (px)</label>
                    <input type="number" name="fbw_width" value="<?php echo esc_attr($width); ?>"
                        min="200" max="1000" style="width:100%;">
                </div>
                <div>
                    <label style="font-size:11px;display:block;margin-bottom:3px;">Alto (px)</label>
                    <input type="number" name="fbw_height" value="<?php echo esc_attr($height); ?>"
                        min="200" max="1200" style="width:100%;">
                </div>
            </div>
            <p style="margin:8px 0 0;font-size:11px;color:#666;">Recomendado: 450 x 600 px</p>
        </div>
    </div>
    <script>
    jQuery(document).ready(function($) {
    ...
    <style>
        ...
        .fbw-select-pdf { height: 40px !important; font-size: 14px !important; }
    </style>
    <?php
}
...
add_action('save_post_vlogger_serie', function ($post_id) {
    ...
    update_post_meta($post_id, '_fbw_pdf_id', isset($_POST['fbw_pdf_id']) ? absint($_POST['fbw_pdf_id']) : 0);
    update_post_meta($post_id, '_fbw_width',  isset($_POST['fbw_width'])  ? absint($_POST['fbw_width'])  : 450);
    update_post_meta($post_id, '_fbw_height', isset($_POST['fbw_height']) ? absint($_POST['fbw_height']) : 600);
});
...
add_filter('the_content', function ($content) {
    ...
    return $content . fbw_render_flipbook(['pdf_url' => $pdf_url, 'width' => $width, 'height' => $height]);
});
...
add_shortcode('flipbook_pdf', function ($atts) {
    $atts = shortcode_atts(['id' => 0, 'url' => '', 'width' => '450', 'height' => '600'], $atts);
    ...
    return fbw_render_flipbook(['pdf_url' => $pdf_url, 'width' => $atts['width'], 'height' => $atts['height']]);
});

function fbw_render_flipbook($args) {
    $args = wp_parse_args($args, ['pdf_url' => '', 'width' => '450', 'height' => '600']);
...
function fbw_enqueue_frontend_assets() {
    ...
    wp_enqueue_style('fbw-flipbook-css', FBW_PLUGIN_URL . 'assets/css/flipbook.css', [], FBW_VERSION);
    wp_enqueue_script('jquery');

    wp_enqueue_script('fbw-pdfjs',
        'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js', [], null, false);

    wp_enqueue_script('fbw-stpageflip',
        FBW_PLUGIN_URL . 'assets/js/page-flip.browser.js', [], '2.0.7', false);

    wp_enqueue_script('fbw-flipbook-js',
        FBW_PLUGIN_URL . 'assets/js/flipbook.js', ['jquery', 'fbw-pdfjs', 'fbw-stpageflip'], FBW_VERSION, true);
